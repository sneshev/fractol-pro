...
static unsigned int calcIterations(Fixed c[2], unsigned int maxIterations) {
...
#include <unistd.h>
// .make x and y ints
// .make WIDTH and HEIGHT ints
// .add operator for Fixed * int, Fixed / int, etc.
void Mandelbrot::draw() {
	Fixed c[2];

	Fixed xStep = (_xRange[MAX] - _xRange[MIN]) / Fixed(WIDTH);
	Fixed yStep = (_yRange[MAX] - _yRange[MIN]) / Fixed(HEIGHT);
	for (int y = 0; y < HEIGHT; y++) {
		c[Y] = _yRange[MIN] + y * yStep;
		for (int x = 0; x < WIDTH; x++) {
			c[X] = _xRange[MIN] + x * xStep;
			unsigned int iterations = calcIterations(c, _maxIterations);
			uint32_t pixelColor = getColor(iterations, _maxIterations);
			mlx_put_pixel(getImg(), x.toInt(), y.toInt(), pixelColor);
